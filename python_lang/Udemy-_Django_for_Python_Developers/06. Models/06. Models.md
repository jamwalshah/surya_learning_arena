...
- When you've created your very first Model project, we've directly started using the MySQL database, if you search for `DATABASES` in `settings.py` file, we've configured the MySQL database and its username, password, etc.
- But, every Django project comes with a `Sqlite` database for free
- Unlike other professional databases like Oracle and MySQL, `db.Sqlite` is a very light-weight database, it doesn't have any server process running, and all the data will go into the file `db.sqlite3`, and when we retrieve the data it'll come from this file, so it uses the file-system, there is no server running
- In this lecture, instructor will beriefly explain what Sqlite is and how to configure it
- Configuring it is already there, if you go to the very first project `firstProject`, open `settings.py`, search for `DATABASES`, and what you see here is configuration for Sqlite by default
- Copy that, and goto `modelDemo` project, goto its `settings.py` file, comment the previous `DATABASES` setting and paste the one which you copied from `firstProject`
- Now lets do the migrations
  - Goto the terminal/command-line, goto the `modelDemo` project directory, and run command below to make migrations, so it'll show message `No changes detected`

  ```bash
  python manage.py makemigrations
  ```

  - Now run below command to migrate the Model into Database, so it applies all the SQL against the Sqlite database

  ```bash
  python manage.py migrate
  ```

- If you want to connect and see the tables inside Sqlite, use below command

  ```bash
  python manage.py dbshell
  ```

  - To see the list of tables, run the below command in Sqlite dbshell

  ```SQL
  .tables
  ```

  - You can then perform all the SQL operations as before, so run a `SELECT` query to show all records from `empApp_employee` table using below command, and it shows no output as there are no records in `empApp_employee` table

  ```SQL
  SELECT * FROM empApp_employee;
  ```

- In this lecture, we've seen how to use the default database which is good for development activities but for production we use Oracle, MySQL, SQL Server, etc.
- Revert back the `DATABASES` property to the settings for `MySQL` database
...
- As an assignment for the Model section, instructor wants you to
  - create a project,
  - within that project configure the MySQL database,
  - You'll then create a `Model` class, that model class is the `Passenger` class which will have
    - firstName
    - lastName
    - email
    - rewardPoints
  - Once you create that Model, instructor wants you to execute the migrations which will create the database in the format `ApplicationName_Passenger` name
    - `makemigrations`
    - `migrate`
  - You'll then create a view, to display the details of the passenger by inserting a record from your MySQL workbench
  - Insert a Passenger record and instructor wants you to render that on the UI through your view
  - Also, want to make that `Passenger` class a part of the `admin` console which you know how to do that within the `admin.py` file to register your `Passenger` Model class
- So create a project from scratch, call it `PassengerDetails` if you want to, within that project there should be an application, within that application, you create a Model called `Passenger`, then you'll configure the MySQL database within your project, Use the Model to generate the table in the database, and then create a `view` and a `template` that will render the passenger details on the UI

### Assignment 2: Solution

- To start virtual Environment, goto `djangoprojects` folder, and open terminal/command-line, and run command below

```bash
env\Scripts\activate
```

- To create project `PassengerDetails`, run command

```bash
django-admin startproject PassengerDetails
```

- To Create an app `passengerApp`
  - change directory to project directory `PassengerDetails` using command
  ...
  ```bash
  python manage.py startapp passengerApp
  ```

- To register your app `passengerApp`
  - Goto `PassengerDetails/PassengerDetails/settings.py` file, and search for `INSTALLED_APPS` property, append your app name to this list as shown below
...
- To configure MySQL database
  - Goto `PassengerDetails/PassengerDetails/settings.py` file, and search for `DATABASES` property, modify it to below settings to be used with MySQL
...
- To create the database `passengers` as specified in `DATABASES` property in `settings.py` file
  - open MySQL Workbench, and run command below to create this database

  ```SQL
  CREATE DATABASE passengers;
  ```

  - Now to check if database is created, use below command to list all the databases

  ```SQL
  SHOW DATABASES;
  ```

- To create a Model class `Passenger` with fields `firstName`, `lastName`, `email` and `rewardPoints`
  - open `PassengerDetails/PassengerDetails/models.py` file, and modify it to as below
...
- To create migrations and execute them
  - Goto terminal/command-line, and goto project directory `PassengerDetails`
  - To make migrations, run command below

  ```bash
  python manage.py makemigrations
  ```

  - To execute the migarations, and generate the tables in database, run below command

  ```bash
  python manage.py migrate
  ```

  - To check if tables are created, goto MySQL Workbench and run commands below to list all the tables
  ...
  - Notice that it has created a table `passengerapp_passengerpatal` which is in the format `<appName>_<modelName>`
- To create a template
  - Create a folder `templates` in project directory `PassengerDetails`, and then create a file `PassengerDetails/templates/passengers.html`
  - You need to register the templates path, open `settings.py` and search for `TEMPLATES`, and then add the templates path in `DIRS` as shown in code below
  ...
  - Now open `PassengerDetails/templates/passengers.html` file, and add HTML with Django template tags & Django template variables as below
...
- To create a view
  - open `PassengerDetails/PassengerDetails/views.py` file and add your view as below
  ...
  def view_passengerData(request):
      ...
      return render(request, 'passengers.html', passengersDict)
  ```

  - Now you need to configure the URLs in `urls.py` file, open `PassengerDetails/PassengerDetails/urls.py` file and add another route in `urlpatterns` list as shown below
  ...
  urlpatterns = [
      path('admin/', admin.site.urls),
      path('passengerData/', views.view_passengerData),
  ]
  ```

- To see this view in action
  - You first need insert some data into empty table using command below

  ```SQL
  INSERT INTO passengerapp_passengerpatal values (1, 'Max', 'Verstappen', 'max@test.com', 650);
  ```

  - Now, open the terminal/command-line and run the command below to run the Django server

  ```bash
  python manage.py runserver
  ```

  - open web-browser and visit [localhost:8000/passengerData](localhost:8000/passengerData), so it'll show you the records from database
- To render this data in Admin UI
  - You first need to create the super-user to access the Admin Panel, open terminal/comman-line, stop the runing server using `Ctrl+D` and then run below command to create the super-user credentials

  ```bash
  python manage.py createsuperuser
  ```

  - Set below credentials for the admin which will be used later
    - Username: `admin`
    - Email: < skip >
    - Password: `admin`
    - Password (again): `admin`
    - Bypass Password validation: `y`
  - Once super-user is created, start the Django server again using command below

  ```bash
  python manage.py runserver
  ```

  - You then need to create a Admin Model class `PassengerAdminPatal(admin.ModelAdmin)` and  register the existing Model class `PassengerPatal(models.Model)` into the `admin.py`, so open the `PassengerDetails/PassengerDetails/admin.py` file and modify it as shown below
  ...
  admin.site.register(PassengerPatal, PassengerAdminPatal)
  ```

  - Now, you've created super-user as well as registered Admin Model class, so you can do CRUD operations on model using Admin console UI
    - Open web-browser, and visit [localhost:8000/admin](localhost:8000/admin), and use the credentials of super-user created recently, in `passengerApp` you can see `PassengerPatal` model on which you can click to see all the `Passengers` from the database
- Now, you can do CRUD operations while logged in as super-user in Admin Console UI
